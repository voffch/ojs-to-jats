...
- Скачиваете готовый JATS XML из меню (например, чтобы выложить на сайт).
...
- Во всплывшем всплывающем окне разрешаете доступ расширению к веб-странице вашего журнала (в зависимости от текущих настроек, этого пункта может не быть).
...
Чекбокс *Обновлять Журнал* делает именно это - подгружает данные в раздел Журнал формы. Но так как из OJS добывается не вся информация (например, нет данных об издателе) и не всегда так как нужно, тумблер по умолчанию выключен. Можете включить при загрузке первой статьи и посмотреть результат.

Чекбокс *Чистить <тэги>* убирает лишние html-тэги из заголовка и аннотации, по умолчанию он включен.

Чекбокс *Определять язык* включен по умолчанию. В некоторых случаях, особенно в OJS 2, несмотря на то, что сайт на двух языках, некоторые данные по определению одноязычны (ФИО авторов) либо могут отсутствовать (например, для англоязычной статьи может не быть русскоязычных названия и аннотации). Включение опции *Определять язык* помогает, для ряда полей, предотвратить заполнение русско- и англоязычных метаданных одинаковым текстом.
